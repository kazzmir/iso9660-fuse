...
def read_iso(iso):
    ...
    with open(iso, 'rb') as file:
        ...
        for x in range(0, 0xf):
            file.read(sector_size)

        def find_primary_volume():
            while True:
                volume = file.read(sector_size)
                type = volume[0]
        ...
        file.seek(root_extent * sector_size)
        root_data = file.read(sector_size)
        ...
        def populate_filesystem(filesystem, data, extent):
            ...
            while offset < len(data):
                ...
                if flags == 2:
                    ...
                    more = {}
                    file.seek(lba * sector_size)
                    populate_filesystem(more, file.read(sector_size), lba)
                    filesystem[name] = Directory(more, data[offset:offset + length])
                else:
                    filesystem[name] = File(data[offset:offset + length])
